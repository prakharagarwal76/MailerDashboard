 ...
 */
public class GetMonthlyServlet extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
         ...
         rentalMap =getMapper();
           ...
           final JSONArray labels=new JSONArray();
               ...
               AggregateIterable<Document> countsPRS=getCounts("PRS",database,collection,serverAddress);
         ...
         labeldate.add(Calendar.DATE,-29);
            ...
            //   System.out.println("PRS MAP "+prsMap.toString());
              // System.out.println("Psm MAP "+psmMap.toString());
               //System.out.println("rental MAP "+rentalMap.toString());
   ...
   // }
     ...
     public AggregateIterable<Document> getCounts(String alertType, String database, String collection,String serverAddress){
...
AggregateIterable<Document> iterable_6_9_p = mongoClient.getDatabase(database).getCollection(collection).aggregate(asList(
        new Document("$match", new Document("sentDate", new Document("$gte",min_13.getTimeInMillis()).append("$lte", current.getTimeInMillis()))
                //.append("isMailSent","Y")                
                .append("isMailSent","Y")
        ),
        new Document("$group", new Document("_id",new Document("date","$sentDate").append("type", "$alertType")).append("count", new Document("$sum", 1)))));
