...
AggregateIterable<Document> iterable_6_9_p = mongoClient.getDatabase(database).getCollection(collection).aggregate(asList(
        new Document("$match", new Document("sentDate", curr_6AM.getTimeInMillis())
        ...
        new Document("$group", new Document("_id", new Document("hour","$sentHour").append("type", "$alertType")).append("count", new Document("$sum", 1)))));
//mongoClient.close();
